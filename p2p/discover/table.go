...
const (
	...
	//refreshInterval    = 10 * time.Minute	// RLS - commented out after merge.
...
func newTable(t transport, db *enode.DB, bootnodes []*enode.Node) (*Table, error) {
	// TODO: Was removed from Ethereum. Reimplement?
	// RLS - If no node database was given, use an in-memory one
	...
	//fmt.Println("[DEBUG] newTable()  bootnodes", bootnodes)
...
func (tab *Table) LookupRandom() []*enode.Node {
	//fmt.Printf("[DEBUG] LookupRandom() \n")
	//fmt.Println("[DEBUG] Nursery:", tab.nursery)
...
func (tab *Table) lookup(targetKey encPubkey, refreshIfEmpty bool) []*node {
	...
	//fmt.Println("[DEBUG] lookup() self", tab.self().ID().TerminalString(), "target", target.TerminalString())
...
func (tab *Table) Closest(target enode.ID, nresults int) ([]*enode.Node) {
